create or replace function public.lji_has_strong_instagram_offer(p_title text)
returns boolean
language sql
immutable
...
as $function$
  with x as (
    select public.lji_norm_text(coalesce(p_title,'')) as t,
           coalesce(p_title,'') as raw
  )
  select
    t ~ '(^| )(vendo|vende se|a venda|alugo|aluga se|para alugar|para locacao|disponivel para venda|disponivel para locacao|valor de venda|valor da locacao)( |$)'
    or (
      t ~ '(^| )(apartamento|apto|casa|sobrado|kitnet|studio|cobertura|terreno)( |$)'
      and raw ~ 'R\$[[:space:]]*[0-9]'
      and t ~ '(mais informacoes|agende|fale com|me chama|chama no|visita|entre em contato|contato:)'
    )
  from x;
$function$;
...
as $function$
  select
    coalesce(p_source_url,'') ~* '(rentola\.com|waa2\.com|achoumudou\.com|mgfimoveis\.com|proprietariodireto\.com\.br|vivareal\.com|zapimoveis\.com|imovelweb\.com|chavesnamao\.com|wimoveis\.com|quintoandar\.com|lista\.mercadolivre\.com\.br)'
