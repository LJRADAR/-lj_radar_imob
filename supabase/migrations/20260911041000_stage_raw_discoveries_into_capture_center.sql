...
begin
  ...
  v_phone := coalesce(
    nullif(v_payload->'attributes'->>'phone',''),
    nullif(v_payload->>'contact_phone','')
  );
  v_whatsapp := coalesce(
    nullif(v_payload->'attributes'->>'whatsapp',''),
    nullif(v_payload->>'whatsapp_url','')
  );
  v_owner_direct := coalesce((new.metadata->'source_router'->>'owner_signal')::boolean, false)
    or lower(coalesce(new.snippet,'')) ~ '(proprietari|direto com o dono|sou o dono|minha casa|meu apartamento)';
  ...
  insert into public.lji_discovered_leads (
    ...
    new.title,
    new.detected_city,
    new.detected_neighborhood,
    new.detected_property_type,
    new.detected_transaction,
    new.advertised_price,
    nullif(v_payload->>'area_m2','')::numeric,
    nullif(v_payload->>'bedrooms','')::integer,
    nullif(v_payload->>'parking_spaces','')::integer,
    new.published_at,
    nullif(v_payload->>'main_image_url',''),
    ...
    left(coalesce(new.snippet,''), 1000),
    v_owner_direct,
    'inbox',
    left(coalesce(new.snippet,''), 3000),
    coalesce(new.first_seen_at, new.created_at, now()),
    now(),
...
drop trigger if exists trg_lji_stage_raw_discovery_to_capture on public.lj_v2_raw_discoveries;
create trigger trg_lji_stage_raw_discovery_to_capture
after insert or update of latest_run_id, raw_payload, metadata, last_seen_at
on public.lj_v2_raw_discoveries
for each row
execute function private.lji_stage_raw_discovery_to_capture();
