...
as $function$
  select
    ...
    or (
      public.lji_norm_text(coalesce(p_source,'')) = 'instagram publico'
      and public.lji_norm_text(split_part(lower(coalesce(p_title,'')), ' no instagram:', 1)) ~ '(restaurante|pizzaria|padaria|confeitaria|receitas|gastronomia)'
      and not (
        public.lji_norm_text(split_part(lower(coalesce(p_title,'')), ' no instagram:', 2))
        ~ '(^| )(venda|vendo|vende se|a venda|aluguel|alugar|aluga se|locacao)( |$)'
      )
    );
$function$;
